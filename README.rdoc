...
The following class methods return a Boolean value:
    ...
    has_identity_at?(identity,date)
    has_identity_at_present?(identity)
    has_unlimited_identity?(identity)
...
===== B. Query methods returning (further-refinable) queries

...

===== C. Query methods that return objects

...

===== D. Methods with limitations
